<script lang="ts">
  ...
  const legalDocument = $derived(getLegalDocument(i18n.locale(), 'privacy'));
  const ogImageUrl = buildOgImageUrl({ type: 'page', slug: 'privacy' });
  const pageDescription = $derived(messages.legal.privacyDescription);
  const structuredData = $derived({
    '@context': 'https://schema.org',
    '@type': 'WebPage',
    name: messages.legal.privacyTitle,
...
<SEO
  ...
  url="/privacy"
  image={ogImageUrl}
  imageAlt={messages.legal.privacyImageAlt}
